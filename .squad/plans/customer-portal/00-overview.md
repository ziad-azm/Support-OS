...
| 46 | [46-story-access-faqs-SUPPORTOS-59.md](46-story-access-faqs-SUPPORTOS-59.md) | Access FAQs | SUPPORTOS-59 | Story 42 (`PORTAL-0`); KB-1/KB-2 (stories 39–40) — one permission-grant migration, no new endpoint |
| 47 | [47-story-submit-feedback-csat-SUPPORTOS-60.md](47-story-submit-feedback-csat-SUPPORTOS-60.md) | Submit Feedback (CSAT) | SUPPORTOS-60 | Story 44 (`PORTAL-2`) — the first genuinely new model in this epic; **completes EPIC 10** |
...
**Verified findings that shaped story 46:**

- **`FAQViewSet`/`ArticleViewSet` hold no customer-scoping logic at all — verified by reading both in full.** FAQs and published articles are org-wide, not per-customer, content — unlike tickets, there is nothing here for `CustomerScopedModelViewSet` to do. The entire backend task is one data migration granting `Permissions.KNOWLEDGE_BASE_VIEW` (never `KNOWLEDGE_BASE_MANAGE`) to the `customer` role — no new viewset, serializer, or endpoint, the same "almost nothing to build" pattern story 45 already demonstrated.
- **`ArticleViewSet.get_queryset`'s existing published-only filter (for any caller lacking `knowledge_base.manage`) already applies to both `list` and `retrieve`**, verified against its own docstring — a draft article's id already 404s for a portal customer with zero new code. This story activates an existing protection; it does not add one.
- **The intake's dependency line (`PORTAL-0, KB-1/2`) deliberately omits `KB-3` (search)** — read as a scope boundary, not an oversight, since every other story in this epic has taken its intake's named dependencies literally. Search remains a forward note for a future story.
- **`features/portal/` keeps its now-established pattern of duplicating thin, portal-shaped read components/types rather than importing from the source feature** — `PortalFaqPage`/`PortalArticleListPage`/`PortalArticleReaderPage`/`PortalMarkdownPreview` all copy `features/knowledge-base/`'s shape, omitting the manage-only "Manage" link and the (for a portal caller, unreachable) draft badge.
- **A latent bug in `HasPermission.has_object_permission`, live since story 42, was found and fixed by this story.** It defaulted `customer_field` to `"customer"` for *any* view once the caller had a `customer_profile`, not only for `CustomerScopedModelViewSet` subclasses that declare it — producing a false `403` on `ArticleViewSet.retrieve` (`Article` has no `customer` relation at all). Unreachable until this story gave a portal customer real `retrieve` access to a plain, non-scoped `BaseModelViewSet` for the first time. Fixed with one `hasattr(view, "customer_field")` guard; `CONVENTIONS.md` §26 updated. `PortalTicketViewSet`'s own scoping (own-ticket 200, other-customer's-ticket 404) verified unaffected.

**EPIC 10 is now fully planned (PORTAL-0 through PORTAL-5).**

**Verified findings that shaped story 47:**

- **The first genuinely new model in this epic.** Every prior PORTAL-N story either extended `Ticket` (PORTAL-1/2/3) or reused `FAQ`/`Article` unmodified (PORTAL-4) — `Feedback` (`apps/tickets/models.py`) is the first new table.
- **`CONVENTIONS.md` §25 already recorded the exact `rating` vocabulary this model needed, from `RPT-4`'s not-yet-built chart design** ("satisfied/neutral/dissatisfied breakdown," a Waffle Chart over those three categories) — the intake's "feeds Reports CSAT" is why the field uses that vocabulary verbatim rather than an arbitrary numeric scale.
- **`Feedback.customer` is a direct, denormalized FK — not reached via `ticket__customer` — specifically because of story 46's own bug fix.** `CustomerScopedModelViewSet`/`HasPermission.has_object_permission` both resolve `customer_field` as a single real attribute (`getattr(obj, f"{customer_field}_id", None)`), not an ORM lookup path; a nested field name would silently break `has_object_permission` the same way the un-guarded default already did before story 46 fixed it — this time as a false negative instead of a false positive.
- **No staff-facing `Feedback` serializer or viewset was built.** `RPT-4` (the eventual reader) is not yet planned — `PortalFeedbackSerializer` is the *only* serializer `Feedback` has, not a narrowed subclass of a staff one the way `PortalTicketSerializer` narrows `TicketSerializer`. Django admin is the interim way to see the data, the same pattern already used for `Role` before SEC-1.
- **`PortalTicketSerializer.has_feedback` reuses the exact `hasattr(...)`-on-a-reverse-`OneToOneField` pattern Story 42 verified for `Customer.user`/`customer_profile`** — one more confirmed instance of `RelatedObjectDoesNotExist` subclassing `AttributeError` making a bare `hasattr`/`getattr` safe with no try/except.
- **Leaving `ticket` un-overridden in `PortalFeedbackSerializer` gets duplicate-feedback prevention for free**, via DRF's auto-derived `UniqueValidator` on a unique field it generates itself — the exact DRF behaviour `apps/customers/serializers.py`'s `CustomerSerializer` had already documented and this story is the second to rely on.
